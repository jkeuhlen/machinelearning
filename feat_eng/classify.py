...
def words_and_char_grams(examples):
    ...
    for quote in quoted:
        result.append(quote)
        words = re.findall("\w+", quote)
        bigrams = re.findall(r"\b\w+\s\w+", quote)
        trigrams = re.findall(r"\b\w+\s\w+\s\w+", quote)
        quadgrams = re.findall(r"\b\w+\s\w+\s\w+\s\w+", quote)
        for q in quadgrams:
            result.append(q)
        for t in trigrams:
            result.append(t)
        ...
        for i in range(len(quote)):
            for j in range (len(quote)):
                result.append(quote[i:i+j])    
    return result

def my_preprocessor(examples): 
...
class Featurizer:
    ...
    def train_feature(self, examples):
        train_counts = self.vectorizer.fit_transform(examples)
        tf_transformer = TfidfTransformer(use_idf=True).fit(train_counts)
        return tf_transformer.transform(train_counts)

    def test_feature(self, examples):
        test_counts = self.vectorizer.transform(examples)
        tf_transformer = TfidfTransformer(use_idf=True).fit(test_counts)
        return tf_transformer.transform(test_counts)
